...
class Calculator::ActiveShipping < Calculator
  ...
  def packages(order)
    ...
    weight = order.line_items.inject(0) do |weight, line_item|
      # uses the particular variant's defined weight or uses a default fallback value
      weight + (line_item.quantity * (line_item.variant.weight || Spree::ActiveShipping::Config[:default_weight]) * multiplier)
    end
